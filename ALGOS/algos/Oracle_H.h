...
void oracle_H_util(struct graph &g ,
vector<bool> visit,
int CC,
int cnode,int goal,
struct node* path,
int& maxora, struct qnode_h& Q, ofstream& f)
{
    ...
    for (int i = 0; i < g.n; i++)
    {
        ...
        if ( g.arr[cnode][i]!=I && g.arr[cnode][i]!=0 && visit[i]==0  )
        {
            ...
            //{
                Q.q.push( { 
                    { CC + g.arr[cnode][i] + g.heur[cnode], CC + g.arr[cnode][i] } 
                    ,
                    { i,{path,visit}  } }); 
    ...
    while (!Q.q.empty())
    {
       ...
       // if(visit[x.second.first]==false)
        ...
        oracle_H_util(g, x.second.second.second, x.first.second, x.second.first, goal, x.second.second.first, maxora , Q, f);
       // else
       // cout<< "oracle excee\n";
    }
}

void oracle_H(struct graph & g, int st, int gl, ofstream& f)
{
    vector <bool> visit(g.n);
    struct node* path = NULL;
    struct qnode_h Q ;
    int maxora = 0 ;
    oracle_H_util( g , visit, 0 , st , gl , path,  maxora, Q, f );
